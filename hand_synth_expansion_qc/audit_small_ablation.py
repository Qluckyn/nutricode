...
def main():
    p=argparse.ArgumentParser(description=__doc__); p.add_argument('--config',type=Path,required=True); p.add_argument('--mode',default='op_i2i',choices=('op_i2i','foundhand_i2i')); p.add_argument('--allow-incomplete',action='store_true'); p.add_argument('--compute-dhash',action='store_true'); a=p.parse_args()
    ...
    root=Path(c['generation_output_root'])/a.mode; records=[]; errors=[]; hashes=defaultdict(list)
